...
- 公开入口页：并发检测候选域名的 `/healthz`，显示推荐入口和手动备用列表。
- 管理后台：单管理员口令登录，支持域名新增、编辑、删除、启停、权重、标签、备注和手动探测。
- SQLite 持久化：保存域名配置、服务端健康检查、匿名浏览器探测结果和审计日志。
- 服务端探测：后台定时从服务器视角探测所有启用域名。
...
入口页会从用户浏览器直接请求该端点。只有允许 CORS，入口页才能准确读取成功状态和延迟。
...
- `PROBE_INTERVAL_MS`：服务端定时探测间隔，默认 60 秒。
